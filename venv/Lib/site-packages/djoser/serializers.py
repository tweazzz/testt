...
class UserCreateSerializer(UserCreateMixin, serializers.ModelSerializer):
    ...
    def perform_create(self, validated_data):
        # Проверяем, по какому API происходит запрос
        is_admin_api = 'admin' in self.context['request'].path

        # Присваиваем роль в зависимости от API
        validated_data['role'] = 'admin' if is_admin_api else 'client'
